...
function fillDataToBulletinListDataTable(dataSet) {
    ...
    $('#bulletin_list_table').DataTable({
        ...
        columns: [
            ...
            {title: "E-Bülten Tipi", data: "type"},
            {title: "Dosya Sayısı", data: "numofdocument"},
            {title: "Başlangıç Tarihi", data: "startdate"},
            {title: "Bitiş Tarihi", data: "enddate"},
            {
                title: 'İşlemler',
                defaultContent: "<button class='btn btn-primary btn-xs btn-tablo-guncelle'><i class='fa fa-pencil'>  Güncelle</i>" +
...
function fillBulletinAddDataTable(dataSet, dataSet2, tableId) {

    if ($.fn.DataTable.isDataTable('#bulletin_update_bulletinContact_table-' + tableId)) {
        $('#bulletin_update_bulletinContact_table-' + tableId).DataTable().destroy();
    }

    var table = $("#bulletin_update_bulletinContact_table-" + tableId).DataTable({
        data: dataSet,
        'columnDefs': [{
            'targets': 0,
            'searchable': false,
    ...
    $('#select-all-check-'+tableId).on('click', function () {
        var rows = table.rows({'search': 'applied'}).nodes();
        $('input[type="checkbox"]', rows).prop('checked', this.checked);
    });

    $('#bulletin_update_bulletinContact_table-'+ tableId +' tbody').on('change', 'input[type="checkbox"]', function () {
        if (!this.checked) {
            var el = $('#select-all-check-'+tableId).get(0);
    ...
    $('#form_bulletin_update-'+tableId).on('click', function (e) {
        var form = this;
...
function fillBulletinContactDataTable(dataSet, tableId) {

    if ($.fn.DataTable.isDataTable('#bulletin_update_bulletinFile_table-' + tableId)) {
        $('#bulletin_update_bulletinFile_table-' + tableId).DataTable().destroy();
    }

    $('#bulletin_update_bulletinFile_table-' + tableId).DataTable({
        data: dataSet,
        "columnDefs" : [
            {
                targets  :   [0],
        ...
        columns: [
            {data: "id"},
            {title: "Dosya Adı", data: "name"},
            {title: "Dosya Tipi", data: "type"},
            {title:'İşlemler', defaultContent:"<button class='btn btn-danger btn-xs btn-tablo-sil'><i class='fa fa-trash-o '>  Sil</i></button>"}
        ],
        "language": {
            "lengthMenu": ' <select>' +
            '<option value="10">10</option>' +
        ...
        fnCreatedRow:function(row,data,index){
            $(row).find('.btn-tablo-sil').on('click',function(){
                if(confirm(data['name']+" isimli dokümanı silmek istediğinizden emin misiniz?")){
                    $.ajax({
                        url : window.location + '/delete_document',
                        type: 'POST',
...
function showguncelle(rowData) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#div_kisi_güncelle-' + id + ' #contactid').val(id);
            $('#div_kisi_güncelle-' + id + ' #update_bulletin_name').val(rowData.name);
            $('#div_kisi_güncelle-' + id + ' #update_bulletin_description').val(rowData.description);
            $('#div_kisi_güncelle-' + id + ' #update_bulletin_type').val(rowData.type);
            $('#div_kisi_güncelle-' + id + ' #update_bulletin_startdate').val(rowData.startdate);
            $('#div_kisi_güncelle-' + id + ' #update_bulletin_enddate').val(rowData.enddate);
        },
        error: function () {
            alert('ajax error!');
        }
